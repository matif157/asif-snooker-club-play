...
<div class="space-y-6 fade-in max-w-2xl">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-white tracking-tight">Edit Table</h1>
            <p class="text-sm text-slate-400 mt-1">Update table #<?= e($table['number']) ?> — <?= e($table['name']) ?></p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="card p-6">
        <form method="POST" action="<?= e(url('/tables/' . $table['id'])) ?>" class="space-y-5">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="type" name="type"
                        ...
                        <option value="Tournament" <?= ($table['type'] ?? '') === 'Tournament' ? 'selected' : '' ?>>Tournament</option>
                    </select>
                </div>
            </div>
